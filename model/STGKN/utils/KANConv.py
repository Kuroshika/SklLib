import torch
import math
from model.STGKN.utils import KANLinear, convolution
...
class KAN_Convolutional_Layer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, update_grid=False):
        ...
        if self.n_convs > 1:
            return convolution.multiple_convs_kan_conv2d(x, self.convs, self.kernel_size[0], self.stride, self.dilation,
...
class KAN_Convolution(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, update_grid=False):
        return convolution.kan_conv2d(x, self.conv, self.kernel_size[0], self.stride, self.dilation, self.padding,
                                      self.device)
